...
function Handle(args) {
    ...
    var pdict = dw.system.Pipeline.execute('AuthorizeCIMNet-Handle', {
        Basket: cart
    });
...
function Authorize(args) {
    ...
    var pdict = dw.system.Pipeline.execute('AuthorizeCIMNet-CreateProfileTransaction', {
        CustomerID: args.Order.getCustomerNo(),
        EmailID : args.Order.getCustomerEmail(),
        CustomerProfileId : session.privacy.CustomerProfileId,
        PaymentProfileId : session.privacy.PaymentProfileId,
        Order : args.Order,
        PaymentInstrument : args.PaymentInstrument
    });

    if (pdict.EndNodeName !== 'done') {
        return {error: true};
    }
