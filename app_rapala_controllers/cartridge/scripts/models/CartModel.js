...
var CartModel = AbstractModel.extend({
        ...
        Transaction.wrap(function () {
            ...
            if (product) {
                ...
                if (productInCart) {
                    ...
                    if(request.httpParameterMap.updateQty.stringValue == 'true'){
    ...
    addBonusProduct: function (bonusDiscountLineItem, product, selectedOptions) {
        ...
        var UpdateProductOptionSelections = require('app_rapala_core/cartridge/scripts/cart/UpdateProductOptionSelections');
    ...
    validateForCheckout: function () {
        var ValidateCartForCheckout = require('app_rapala_core/cartridge/scripts/cart/ValidateCartForCheckout');
    ...
    consolidateInStoreShipments: function () {
        ...
        for (var i = 0; i < plis.length; i++) {
            ...
            if (pli.shipment.shippingMethod) {
            ...
            while (productIter.hasNext()) {
                ...
                if (product) {
                    ...
                    if (psc && psc.getAmount() && psc.isSurcharge()) {
                        ...
                        surchargeTotal = surchargeTotal.add(psc.getAmount().multiply(pli.getQuantity()));
                        adustedSurchargeTotal = adustedSurchargeTotal.add(psc.getAmount().multiply(pli.getQuantity()));
                    }
    ...
    getNonGiftCertificateAmount: function () {
        ...
        var allotmentPIList = this.getPaymentInstruments("ALLOTMENT");
    	var iterTemp = allotmentPIList.iterator();
    ...
    calculatePaymentTransactionTotal: function () {
        ...
        var giftCertTotal = new Money(0.0, this.getCurrencyCode());
        var allotmentTotal= new Money(0.0, this.getCurrencyCode());
        var OPT = new Money(0.0, this.getCurrencyCode());
        ...
        while (iter.hasNext()) {
            ...
            if( paymentInstrument.paymentMethod == "ALLOTMENT" )
        	{
        		allotmentTotal = allotmentTotal.add(paymentInstrument.getPaymentTransaction().getAmount());
        		continue;
        	}
        ...
        if (!nonGCPaymentInstrument) {
            ...
            if (allotmentTotal.value + giftCertTotal.value < orderTotal.value) {
                return false;
            } else {
                return true;
            }
    ...
    createGiftCertificatePaymentInstrument: function (giftCertificate) {
        ...
        var allPaymentInstrs = this.getPaymentInstruments("ALLOTMENT").iterator();
        var existingPI = null;
...
/**
 * PREVAIL - 3rd party tax calculations.
function calculateTax() {
	if (dw.system.Site.getCurrent().getCustomPreferenceValue('isEndToEndMerchant') ||
		dw.system.Site.getCurrent().getCustomPreferenceValue('taxService').toString() === 'VERTEX_O') {
		dw.system.Pipeline.execute('Vertex-Vertex_O', {
			Basket : CartModel.goc().object
		});
	} else if (dw.system.Site.getCurrent().getCustomPreferenceValue('taxService').toString() === 'AVALARA') {
		dw.system.Pipeline.execute('Avatax-CalculateTaxes', {
			Basket : CartModel.goc().object
		});
	}
}*/
