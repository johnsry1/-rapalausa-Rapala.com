...
var ProfileModel = AbstractModel.extend(
    /** @lends module:models/ProfileModel~ProfileModel.prototype */
    {
         ...
         */
        addAddressToAddressBook: function (addressid,addressToAdd) {
            ...
            return Transaction.wrap(function () {
                var address;
                if (addressToAdd) {
                    var usedAddress, addressUsed;
                    usedAddress = addressToAdd;
                    //Checks if the address already exists in the address book
                    for (var i = 0; i < addressBook.addresses.length; i++) {
                        addressUsed = addressBook.addresses[i];
                        if (addressUsed.isEquivalentAddress(addressToAdd)) {
                            address = addressUsed;
                            break;
                        }
                    }
                    ...
                    if (!address) {
	                    if(addressid != null && !empty(addressid)){
	                		...
	                		if(address == null)
	                		{
	                			address = addressBook.getAddress(addressid.toUpperCase());
	                		}
	                		if(address == null)
	                		{
	                			address = addressBook.getAddress(addressid.toLowerCase());
	                		}
	                	}
	                	else if(usedAddress.city !=null && !empty(usedAddress.city)){
	                		address = addressBook.getAddress(usedAddress.city);
	                		if(address == null)
	                		{
	                			address = addressBook.getAddress(usedAddress.city.toUpperCase());
	                		}
	                		if(address == null)
	                		{
	                			address = addressBook.getAddress(usedAddress.city.toLowerCase());
	                		}
	                	}
	                	else{	
	                		address = that.getEquivalentAddress(usedAddress);
	                	}
	                    ...
	                    if (!address) {
	                        ...
	                        if(addressid != null && !empty(addressid)){
	                			var addressID : String = that.determineUniqueAddressID(addressid.toUpperCase(), addressBook);
	                		}
	                        else if(usedAddress != null){
	                			var addressID : String = that.determineUniqueAddressID(usedAddress.city.toUpperCase(), addressBook);
	                		}	
        ...
        determineUniqueAddressID: function (city) {
            var accountUtils = require('app_rapala_core/cartridge/scripts/account/Utils');
        ...
        */
        getEquivalentAddress: function(usedAddress){
        	var accountUtils = require('app_rapala_core/cartridge/scripts/account/Utils');
        	 return accountUtils.getEquivalentAddress(this.getAddressBook(), usedAddress);
        },
