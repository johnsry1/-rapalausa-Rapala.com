...
function show() {
    ...
    var recommendations = require('app_rapala_core/cartridge/scripts/cart/CalculateCartRecommendations.ds').recommendationList(dw.system.Session);
    app.getView('Cart', {
    	RecommendetionList : recommendations,
        cart: basket,
...
function submitForm() {
    ...
    formResult = cartForm.handleAction({
        ...
        'addCoupon': function (formgroup) {
            ...
            var cpnname = formgroup.couponCode.htmlValue.toUpperCase()
            if (cpnname) {

                status = cart.addCoupon(cpnname);
...
function miniCart() {
    ...
    app.getView({
    	...
    	prodNetPrice : cart ? priceVals[0] :null
...
function addToWishlist() {
    ...
    //JIRA PREV-539 : On click of Add to wishlist in cart page is navigating to error page. Removed .object
    product = Product.get(productID);
    productOptionModel = product.updateOptionSelection(request.httpParameterMap);

    productList = app.getModel('ProductList').get();
    /*JIRA PREV-539 : On click of Add to wishlist in cart page is navigating to error page.
    Changed product.object to product*/
...
function addCouponJson() {
    ...
    if (request.httpParameterMap.format.stringValue === 'ajax') {
        //let r = require('~/cartridge/scripts/util/Response');
       // r.renderJSON(template);
    var NewCouponLineItem = require('app_rapala_core/cartridge/scripts/checkout/GetCouponLine.ds').getCouponLineItem(cart.object, couponCode);
       app.getView({
    	   Basket : cart.object,
    	   NewCouponLineItem : NewCouponLineItem,
    	   CouponStatusCode : couponStatus.CouponStatus
       }).render('checkout/billing/couponapplyjson');
    }
}

function editLineItem(){
	var product, productOptionModel, cart;
	var lineItem = null;
    product = app.getModel('Product').get(request.httpParameterMap.pid.stringValue);
    productOptionModel = product.updateOptionSelection(request.httpParameterMap);
    cart = app.getModel('Cart').get();
    
    var plItems = cart.object.getProductLineItems();
    for each(var plItem in plItems){
    	if(request.httpParameterMap.lineItemId.stringValue == plItem.getUUID()){
    		lineItem = plItem;
    	}
    }
    if(lineItem != null){
	    Transaction.wrap(function () {
	        cart.updateLineItem(lineItem, product.object, request.httpParameterMap.Quantity.doubleValue, productOptionModel);
	        cart.calculate();
	    });
...
/** Adds bonus product to cart.
 * @see {@link module:controllers/Cart~addBonusProductJson} */
exports.EditLineItem = guard.ensure(['post'], editLineItem);
