...
function returnToForm(cart, params) {
    ...
    if (!empty(cart.getPaymentInstruments()) &&
        cart.getPaymentInstruments(dw.order.PaymentInstrument.METHOD_GIFT_CERTIFICATE).length > 0) {
        app.getForm('billing').copyFrom({
            giftCertCode: cart.getPaymentInstruments(dw.order.PaymentInstrument.METHOD_GIFT_CERTIFICATE)[0].giftCertificateCode
        });
    }
    ...
    if (params && ppalError == null) {
        app.getView(require('~/cartridge/scripts/object').extend(params, {
            ...
            PlaceOrderError : params.PlaceOrderError,
...
function handlePaymentSelection(cart) {
    ...
    // skip the payment handling if the whole payment was made using allotment amount
    if (app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value.equals("ALLOTMENT")) {
        result = {
            ok: true
        };
    }
...
function redeemGiftCertificateJson() {
    ...
    giftCertCode = request.httpParameterMap.giftCertificateID.stringValue;
    ...
    if (request.httpParameterMap.format.stringValue !== 'ajax') {
    	...
    	app.getView({
    		NewGCPaymentInstrument : giftCertStatus.details.NewGCPaymentInstrument,
    		GiftCertStatus : giftCertStatus,
    		Basket : cart.object
    	}).render('checkout/giftcert/giftcertredeemjson');
    }
}
...
function removeGiftCertificate() {
    ...
    var giftCertRemoveStatus = new Status(Status.OK);
    app.getView({
    	GiftCertRemoveStatus : giftCertRemoveStatus
    }).render('checkout/giftcert/giftcertremovejson');
}
...
function getGiftCertificateBalance() {
    ...
    if (giftCertificate && giftCertificate.isEnabled()) {
    	...
    	}).render('checkout/giftcert/giftcertdetailsjson');*/
    } else {
        responseUtils.renderJSON({
...
function handlePayPalResponse() {
    ...
    var pdict = dw.system.Pipeline.execute('Paypal-HandlePayPalResponse');
...
function updatePaymentForms(validPaymentInstruments, paymentAmount){
	var applicablePaymentMethods, applicablePaymentCards, countryCode;
	countryCode = Countries.getCurrent({
        CurrentRequest: {
            locale: request.locale
        }
	...
	for each(var pi in validPaymentInstruments){
		if(pi.paymentMethod == PaymentInstrument.METHOD_CREDIT_CARD){
			...
			app.getForm('billing').object.paymentMethods.creditCard.owner.value = pi.creditCardHolder;
			app.getForm('billing').object.paymentMethods.creditCard.type.value = pi.creditCardType;
			app.getForm('billing').object.paymentMethods.creditCard.expiration.month.value = pi.creditCardExpirationMonth;
			app.getForm('billing').object.paymentMethods.creditCard.expiration.year.value = pi.creditCardExpirationYear;
		}
	}
}
...
function removeCoupon(){
     ...
     cart.calculate();
     /*
      * Adjust gift certificate redemptions as after applying/removing coupon(s),
      * order total is changed. AdjustGiftCertificate pipeline removes and
      * then adds currently added gift certificates to reflect order total
      * changes.
      */
     adjustGiftCertificates();
     if(customer.authenticated){
    	 require('app_rapala_core/cartridge/scripts/prostaff/UseProStaffAllowance.ds').checkAllotmentPayment(customer,cart.object);
     }
	 cart.calculate();
	 });
...
function getOrderTotalJson(){
    ...
    app.getView({
            Basket : cart.object
    }).render("checkout/billing/getordertotaljson");
...
function displayCoupon(){
	 var cart = app.getModel('Cart').get();
	    app.getView({
	            Basket : cart.object
	    }).render("checkout/billing/coupondisplay");
}
...
function returnToBIlling(placeOrderError){
	var cart = app.getModel('Cart').get();
	 if (cart) {
...
exports.HandlePayPalResponse = guard.ensure(['https', 'get'], handlePayPalResponse);
/** Handles the response from PayPal
 * @see module:controllers/COBilling~ */
exports.UpdateCartSummary = guard.ensure([], updateCartSummary);
/**Handles the Remove coupon
 * @see module:controllers/COBilling~removeCoupon */
exports.RemoveCoupon = guard.ensure(['https'], removeCoupon);
/** Refreshes Payment Methods
 * @see module:controllers/COBilling~refreshPaymentMethods */
exports.RefreshPaymentMethods = guard.ensure([], refreshPaymentMethods);
/** Gets the Order Total
 * @see module:controllers/COBilling~getOrderTotalJson */
exports.GetOrderTotalJson = guard.ensure([], getOrderTotalJson);
/** Display the addded coupon message in billing page
 * @see module:controllers/COBilling~displayCoupon */
exports.DisplayCoupon = guard.ensure([], displayCoupon);
/** Remove CC payment instrument
* @see module:controllers/COBilling~RemoveCCPaymentInstrument */
exports.ClearCCForm = guard.ensure([], clearCCForm);
/** Billing redirect
