...
var RateLimiter    = require('app_rapala_core/cartridge/scripts/util/RateLimiter');
...
function show() {
    ...
    var existing = false;
    if(!empty(request.httpParameterMap.existing) && request.httpParameterMap.existing.value != null){
    	existing = request.httpParameterMap.existing.value;
    }
    
    var v = app.getView('Login',{
        RegistrationStatus: false,
        ExistedMail : existing
    });
...
function handleLoginForm () {
    ...
    loginForm.handleAction({
        ...
        search: function (form, action) {
            //PREVAIL - Added support to search wishlists in Login page.
            var ProductList;
            if (action.htmlName.indexOf('wishlist') !== -1) {
                ProductList = app.getModel('ProductList');
