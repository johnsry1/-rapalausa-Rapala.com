...
function editEvent() {
    Form.get('giftregistry').handleAction({
        ...
        setBeforeAfterAddresses: function(form) {
            var assignEventAddresses = require('app_rapala_core/cartridge/scripts/account/giftregistry/AssignEventAddresses');
            var productList = ProductList.get(form.object);
...
function replaceProductListItem() {
    ...
    var ScriptResult = new Pipelet('Script', {
        ...
        ScriptFile: 'app_rapala_core:account/ReplaceProductListItem.ds'
    }).execute({
        ProductList: ProductList,
 ...
 */
//PREV-526 -Click on Add to Gift Registry in Quick view / PDP is not navigating to gift registry login page. Added {scope: 'giftregistry'}
exports.AddProduct = guard.ensure(['get', 'https', 'loggedIn'], addProduct, {scope: 'giftregistry'});
