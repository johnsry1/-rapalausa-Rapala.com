...
function wishListForm() {
    ...
    app.getForm('wishlist').handleAction({
        ...
        deleteItem: function (formgroup, action) {
            /*JIRA PREV-559 - On removing the single item from wishlist page, all the items are getting removed.
            Replaced remove with removeItem.*/
...
/**
 * Called through Ajax for authenticated users to add product 
 * to wislist
 */
function addAjax() {
	addProduct();
...
function setShippingAddress() {
    ...
    if (addressId) {
        //PREVAIL - corrected API issues.
        address = customer.getAddressBook().getAddress(addressId);
...
exports.AddAjax = guard.ensure(['get'], addAjax, {scope: 'wishlist'});
