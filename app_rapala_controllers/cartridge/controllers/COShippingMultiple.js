    ...
    multiShippingForm.handleAction({
        save: function () {
            ...
            var result = Transaction.wrap(function () {
                var MergeQuantities = require('app_rapala_core/cartridge/scripts/checkout/multishipping/MergeQuantities');
        ...
        //JIRA PREV-94 : No option to come back to single page shipping from multiple shipping page. Added shipToSingle action.
        shipToSingle: function () {
            new dw.system.Pipelet('Script', {
                Transactional: true,
                OnError: 'PIPELET_ERROR',
                ScriptFile: 'app_rapala_core:checkout/MergeToDefaultShipment.ds'
            }).execute({
...
function addEditAddress() {
    ...
    if (!app.getForm(session.forms.multishipping.editAddress.addressFields).copyTo(newAddress) || !app.getForm(session.forms.multishipping.editAddress.addressFields.states).copyTo(newAddress)) {
        ...
        return {success: true, address: newAddress}; //PREVAIL - Corrected typo. sucess to success
    }
}
...
/**
 * Updates mini-summary when a shipping methods is selected.
 * Fixes following defects.
 * JIRA PREV-99 : shipping methods is not displayed for 2nd address in right nav
 * JIRA PREV-103 : Selected shipping method will not update automatically in right nav
 */
function selectShippingMethod() {
    ...
    new dw.system.Pipelet('Script', {
        Transactional: true,
        OnError: 'PIPELET_ERROR',
        ScriptFile: 'checkout/multishipping/UpdateShipmentShippingMethod.ds'
    }).execute({
        Shipment: cart.getShipment(request.httpParameterMap.shipmentID.stringValue),
...
/** Updates mini-summary when a shipping methods is selected.
 * @see module:controllers/COShippingMultiple~selectShippingMethod */
