...
exports.calculate = function (basket) {
    ...
    // ===================================================
	var stateCode = null;
	if(!empty(request.httpParameterMap.selectedState) && request.httpParameterMap.selectedState.value != null){
		stateCode = request.httpParameterMap.selectedState.value;
	}
    ...
    if(Site.getCurrent().preferences.custom.hasOwnProperty('ATEnable') &&   Site.getCurrent().getCustomPreferenceValue('ATEnable')) {
        ...
        if (response == null || (response!=null && (!response.OK && !response.noServiceCall))) {
 ...
 */
function calculateTax (basket, stateCode) {
    var shipments = basket.getShipments().iterator();
    while (shipments.hasNext()) {
        var shipment = shipments.next();
