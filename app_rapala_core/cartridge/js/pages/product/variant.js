...
var swatchesSelected = {
		...
		init: function() {
			...
			if( $sizeContainer.find("ul li.selected").length > 0 ) {
				...
				$('#pdpMain').find(".variationattributes .swatches,.variationattributes .variantdropdown").each(function(i) {
					var $this = $(this);
					if(!$this.hasClass("size")) {
						$this.addClass("future");
						$this.find(".optionwrapper").slideUp(400);
...
function initializeEvent() {
	...
	$(".variationattributes").on("click",".selected a.filter",function(e){
		...
		$this.nextAll('.swatches, .variantdropdown').each(function(i){
			...
			if( $variant.hasClass("future") ) {
				...
				if($variant.hasClass("swatches")) {
					...
					/*if($variant.find(".unselectURL").length > 0) {
						$select.val($variant.find(".unselectURL").val()).trigger("change");
					}else {
						$select.val($select.find("option.emptytext").attr("value")).trigger("change");
					}*/
				}
	 ...
	 $pdpMain.find(".variationattributes .variantdropdown").each(function(i) {
			var $this = $(this);
...
/*    $pdpMain.find(".variationattributes .variantdropdown").each(function(i) {
		var $this = $("body").find(".variationattributes .variantdropdown").eq(i);
		var $select = $variant.find("select");
		if( $this.find("select option").length < 3 ) {
			$select.val($variant.find(".unselectURL").val()).trigger("change");
		}
    });	
*/
