...
exports.init = function () {
    ...
    $('select[name$="_addressList"]', $form).on('change', function () {
        ...
        var addressHTML = selectedAddress.title + ' ' + selectedAddress.firstName + ' ' + selectedAddress.lastName + '<br/>' 
            + selectedAddress.address1 + ' ' + selectedAddress.address2 + '<br/>' 
            + selectedAddress.city + ' ' + selectedAddress.stateCode + ' ' + selectedAddress.postalCode + '<br/>' 
            + selectedAddress.countryDisplayValue + '<br/>' 
            + selectedAddress.phone;
        if (selectedAddress.countryCode.toLowerCase() == 'gb' || selectedAddress.countryCode.toLowerCase() == 'uk') {
            addressHTML = selectedAddress.title + ' ' + selectedAddress.firstName + ' ' + selectedAddress.lastName + '<br/>' 
            + selectedAddress.address1 + ' ' + selectedAddress.address2 + '<br/>' 
            + selectedAddress.city + ' ' + selectedAddress.postalCode + '<br/>'
            + selectedAddress.stateCode + ' ' + selectedAddress.countryDisplayValue + '<br/>' 
            + selectedAddress.phone;    
        } 
        $('.selected-shipping-address').append(addressHTML);
        if ($('.checkout').hasClass('billingsection')) {
